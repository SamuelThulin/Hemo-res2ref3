...
// new effects
let amVib, amVerb, fmVerb, autoFilt1, pingPong1, fbDelay1;
...
function setup() {
  ...
  textFont(hussar);
  textSize(fontSize);
  points = hussar.textToPoints(String(5.3), width / 3, height / 1.2, fontSize, {
    sampleFactor: 0.8
  })
  ...
  chooseVal.position(60, 10).style('font-family', 'Arial').style('font-size', '12px');
...
//now I need to DO something with these arrays. Let's start by having 3 different synth instruments; the first item [0] in the array will determine fundamental frequency (will apply to all instruments somehow); the following pairs of items will determine pairs of parameters for each of the 3 synths. Once I figure this out, I can get more complex

//place-holder for what users hear when they first arrive - ideally a random selection from the bgDict
function keyPressed() {
  amSynth.chain(amVib, amVerb, Tone.Destination);
  fmSynth.chain(fmVerb, Tone.Destination);
  amSynth2.chain(amVib, autoFilt1.start(), Tone.Destination);
  fmSynth2.chain(autoFilt1, Tone.Destination);
  ...
  // loop1 = setInterval(null, 2000);
  // loop2 = setInterval(null, 3000);
  // loop3 = setInterval(null, 4000);
  // loop4 = setInterval(null, 2000);
  // loop5 = setInterval(null, 3000);
  // loop6 = setInterval(null, 4000);
  // loop7 = setInterval(null, 2000);
  // loop8 = setInterval(null, 3000);
...
function valSelectEvent() {
  ...
  function amSynthLoopTarget() {
    ...
    fmSynth.detune.value = bgDict.get(selBg)[1];
    ...
    amSynth2.detune.value = bgDict.get(selBg)[1];
  ...
  function fmSynth2LoopTarget() {
    fmSynth2.detune.value = bgDict.get(selBg)[1];
...
if(8.0>bgDict.get(selBg)[0] && bgDict.get(selBg)[0]>=4.0){
